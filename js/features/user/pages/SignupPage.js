 ...
 * Dedicated signup page with animations and validation
 */
export function SignupPage({ onSignupSuccess }) {
    ...
    const mainContainer = createElement('div', {
        className: 'relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-10 transition-all duration-700 ease-in-out'
    });
    ...
    const submitBtn = createElement('button', {
        type: 'submit',
        className: 'bg-gradient-to-r from-blue-600 to-blue-800 text-white font-semibold py-3 px-12 rounded-lg mt-4 uppercase text-xs tracking-wider hover:shadow-lg hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed btn-ripple w-full'
    });
    ...
    // Form submit handler
    form.addEventListener('submit', (e) => handleSignUpSubmit(e, form, submitBtn, onSignupSuccess));
 ...
 */
async function handleSignUpSubmit(e, form, submitBtn, onSignupSuccess) {
    e.preventDefault();
    ...
    try {
        // Attempt registration (using imported function)
        ...
        // Call success callback to navigate to login
        if (onSignupSuccess) {
