    ...
    function renderContent() {
        ...
        content.innerHTML = `
            <div class="grid grid-cols-1 gap-6">
                ...
                <div id="bidding-interface-container" class="lg:sticky lg:top-6 h-fit">
                    <!-- BiddingInterface component will be inserted here -->
                </div>
            </div>
        `;
        ...
        const biddingContainer = content.querySelector('#bidding-interface-container');
        if (biddingContainer && plateData.auctionId) {
            // Create auction object for BiddingInterface
            const auctionData = {
                id: plateData.auctionId,
                plateNumber: plateData.plateNumber,
                name: plateData.plateNumber,
                startingPrice: parseFloat(plateData.startPrice.replace(/[^\d.]/g, '')),
                currentBid: plateData.currentBid || parseFloat(plateData.startPrice.replace(/[^\d.]/g, '')),
                currentBidder: plateData.currentBidder || null,
                bidCount: plateData.bidCount || 0,
                status: plateData.status || 'upcoming', // 'upcoming', 'live', 'ended'
                registeredUsers: plateData.registeredUsers || [],
                province: plateData.province,
                type: plateData.type
            };

            const biddingInterface = BiddingInterface({ auction: auctionData });
            biddingContainer.appendChild(biddingInterface);
        }
