...
export function NotificationDetailPage({ id, onNavigate } = {}) {
    const item = notifData.find(n => n.id === id) || null;

    const container = createElement('div', { className: 'container mx-auto px-4 py-12' });

    if (!item) {
        const notFound = createFromHTML(`<div class="text-center py-16"><h2 class="text-2xl font-bold">Thông báo không tồn tại</h2><p class="text-gray-500 mt-4">Thông báo bạn tìm không tồn tại hoặc đã bị xóa.</p></div>`);
    ...
    const html = `
        <div class="max-w-4xl mx-auto bg-white p-6 rounded shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <div>
                    <button class="px-3 py-2 border rounded text-sm text-gray-600 hover:bg-gray-100" id="back-to-notifs">Quay lại</button>
                </div>
            </div>
    ...
    const backBtn = container.querySelector('#back-to-notifs');
    if (backBtn && typeof onNavigate === 'function') {
        // Navigate back to News page with Notifications tab active
        backBtn.addEventListener('click', () => onNavigate('news', { initialTab: 'notif' }));
    }
